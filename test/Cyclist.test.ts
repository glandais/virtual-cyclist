...
describe('Cyclist', () => {
    ...
    describe('withModifications', () => {
        test('should create modified cyclist with single parameter change', () => {
            const originalCyclist = new Cyclist(80, 250, false, 0.6, 0.7, 0.5, 35, 60);
            const modifiedCyclist = originalCyclist.withModifications({ power: 300 });

            expect(modifiedCyclist.mKg).toBe(80);
            expect(modifiedCyclist.power).toBe(300);
            expect(modifiedCyclist.harmonics).toBe(false);
            expect(modifiedCyclist.maxBrakeG).toBe(0.6);
            expect(modifiedCyclist.cd).toBe(0.7);
            expect(modifiedCyclist.a).toBe(0.5);
            expect(modifiedCyclist.maxAngleDeg).toBe(35);
            expect(modifiedCyclist.maxSpeedKmH).toBe(60);
            expect(modifiedCyclist).not.toBe(originalCyclist);
        });

        test('should create modified cyclist with multiple parameter changes', () => {
            const originalCyclist = new Cyclist(80, 250, false, 0.6, 0.7, 0.5, 35, 60);
            const modifiedCyclist = originalCyclist.withModifications({
                mKg: 75,
                power: 320,
                harmonics: true,
                maxSpeedKmH: 70,
            ...
            expect(modifiedCyclist.mKg).toBe(75);
            expect(modifiedCyclist.power).toBe(320);
            expect(modifiedCyclist.harmonics).toBe(true);
            expect(modifiedCyclist.maxBrakeG).toBe(0.6);
            expect(modifiedCyclist.cd).toBe(0.7);
            expect(modifiedCyclist.a).toBe(0.5);
            expect(modifiedCyclist.maxAngleDeg).toBe(35);
            expect(modifiedCyclist.maxSpeedKmH).toBe(70);
        });

        test('should create modified cyclist with all parameters changed', () => {
            const originalCyclist = new Cyclist(80, 250, false, 0.6, 0.7, 0.5, 35, 60);
            const modifiedCyclist = originalCyclist.withModifications({
                mKg: 70,
                power: 350,
            ...
            expect(modifiedCyclist.mKg).toBe(70);
            expect(modifiedCyclist.power).toBe(350);
            expect(modifiedCyclist.harmonics).toBe(true);
            expect(modifiedCyclist.maxBrakeG).toBe(0.8);
            expect(modifiedCyclist.cd).toBe(0.6);
            expect(modifiedCyclist.a).toBe(0.4);
            expect(modifiedCyclist.maxAngleDeg).toBe(40);
            expect(modifiedCyclist.maxSpeedKmH).toBe(75);
        });

        test('should create identical cyclist with empty modifications', () => {
            const originalCyclist = new Cyclist(80, 250, false, 0.6, 0.7, 0.5, 35, 60);
            const modifiedCyclist = originalCyclist.withModifications({});

            expect(modifiedCyclist.mKg).toBe(originalCyclist.mKg);
            expect(modifiedCyclist.power).toBe(originalCyclist.power);
            expect(modifiedCyclist.harmonics).toBe(originalCyclist.harmonics);
            expect(modifiedCyclist.maxBrakeG).toBe(originalCyclist.maxBrakeG);
            expect(modifiedCyclist.cd).toBe(originalCyclist.cd);
            expect(modifiedCyclist.a).toBe(originalCyclist.a);
            expect(modifiedCyclist.maxAngleDeg).toBe(originalCyclist.maxAngleDeg);
            expect(modifiedCyclist.maxSpeedKmH).toBe(originalCyclist.maxSpeedKmH);
            expect(modifiedCyclist).not.toBe(originalCyclist);
        });
    });
