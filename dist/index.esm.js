const kt = 9.8, fe = 0.5555555555555556, te = 1, Ae = 6378137, ue = 0.0066943799901378, de = Ae * 2 * Math.PI, ye = 4e-3, Se = 0.05, Le = 0.07, xe = 0.7, _e = 0.976, De = 80, Ie = 280, be = 0.6, Ce = 35, zt = Ce * Math.PI / 180, Oe = 100, We = 0.7, Fe = 0.5, Ge = 1.225, O = class {
};
...
const _ = (a) => new Be(a), A = _("tile/cache/ReentrantLock");
class ke {
  // ========================================================================
  // CONSTRUCTOR
  // ========================================================================
  ...
  async acquire(e, t) {
    ...
    A.debug("%s: Lock creating new promise", e);
    const s = (async () => {
      try {
        A.debug("%s: Promise executing function", e);
        const i = await t();
        return A.debug("%s: Promise resolved successfully", e), i;
      } catch (i) {
        throw A.error("%s: Promise rejected - %o", e, i), i;
      } finally {
        A.debug("%s: Promise cleanup - removing lock and releasing slot", e), this.locks.delete(e), this.releaseLoadingSlot(e);
      }
    })();
    return this.locks.set(e, s), A.debug("%s: Lock registered promise (total locks: %d)", e, this.locks.size), s;
  }
  // ========================================================================
  ...
  async acquireLoadingSlot(e) {
    ...
    return A.debug(
      ...
      this.maxConcurrent,
      this.waitQueue.length
    ), A.timeLevel(E.DEBUG, e), new Promise((t) => {
      this.waitQueue.push(() => {
        A.timeEndLevel(E.DEBUG, e), this.loadingCount++, A.debug(
...
class Xe {
  ...
  async checkCache() {
      ...
      return this.cache;
    {
      ...
      const t = (n) => n.close(), s = new $e(this.tileUrlTemplate, e), i = new ze(
        this.cacheSize,
        (n) => `${n.z}/${n.x}/${n.y}`,
        (n) => s.loadTile(n),
        t
...
function k(a) {
  ...
  let i = s.x, n = s.y;
  const r = s.z;
  e < 0 && (e += N, i -= 1), e >= N && (e -= N, i += 1), t < 0 && (t += N, n -= 1), t >= N && (t -= N, n += 1);
...
function pe(a, e) {
  const t = Re(a, e), s = Math.floor((t.xFloat - t.x) * N), i = Math.floor((t.yFloat - t.y) * N);
  return {
    tile: {
      z: e,
      x: t.x,
    ...
    x: Math.max(0, Math.min(N - 1, s)),
    y: Math.max(0, Math.min(N - 1, i))
  };
}
class Qe {
  constructor(e) {
...
class x {
  ...
  static haversine(e, t) {
    const s = e.latitude * $.DEG_TO_RAD, i = t.latitude * $.DEG_TO_RAD, n = (t.latitude - e.latitude) * $.DEG_TO_RAD, r = (t.longitude - e.longitude) * $.DEG_TO_RAD, o = Math.sin(n / 2) * Math.sin(n / 2) + Math.cos(s) * Math.cos(i) * Math.sin(r / 2) * Math.sin(r / 2), l = 2 * Math.atan2(Math.sqrt(o), Math.sqrt(1 - o));
  ...
  static totalPathDistance(e) {
    ...
    let t = 0;
    for (let s = 1; s < e.length; s++)
      t += x.haversine(e[s - 1], e[s]);
    return t;
  }
}
const X = _("utils/ElevationSmoother");
class Ze {
  /**
  ...
  static smooth(e, t = 50) {
    if (X.debug("smooth %s", e.length), e.length < se.MIN_SMOOTHING_POINTS)
      return X.debug("too small"), e;
    if (t <= 0)
      throw new Error(`Invalid window size: ${t}. Must be positive`);
    X.timeLevel(E.INFO, "smooth");
    const s = x.cumulativeDistances(e), i = [];
    ...
    return X.timeEndLevel(E.INFO, "smooth"), i;
  }
  /**
...
class tt {
  ...
  constructor(e = {}) {
    ...
    }, et.dir("Config :", this.config), this.validateConfig(), this.tileManager = new Xe(this.config.tileUrlTemplate, this.config.cacheSize), this.calculator = new Qe(this.tileManager), this.batchCalculator = new Je(this.calculator);
  }
  /**
...
const y = _("tile/fetcher/CanvasPool");
class st {
  constructor(e) {
  ...
  acquire() {
    ...
    return e ? y.debug(
      ...
      this.totalAcquired
    ) : (e = this.builder(), this.totalCreated++, y.debug(
   ...
   */
  release(e) {
    e ? (this.totalReleased++, this.available.push(e), y.debug(
      "Canvas released to pool (pool size: %d → %d, total released: %d)",
      this.available.length - 1,
      this.available.length,
      this.totalReleased
    ), this._resetIdleTimer()) : y.warn("Canvas release attempted with null/undefined canvas");
  }
  /**
   * Reset the idle timer for automatic cleanup
   */
  _resetIdleTimer() {
    this.idleTimer ? (clearTimeout(this.idleTimer), y.debug("Idle timer reset - previous timer cleared")) : y.debug("Idle timer started - %d ms until auto-trim", this.idleTimeout), this.idleTimer = setTimeout(() => this._trim(), this.idleTimeout);
  }
  /**
...
var h = /* @__PURE__ */ ((a) => (a[a.LAT = 0] = "LAT", a[a.LON = 1] = "LON", a[a.ELE = 2] = "ELE", a[a.BEARING = 3] = "BEARING", a[a.DIST = 4] = "DIST", a[a.RADIUS = 5] = "RADIUS", a[a.TIME = 6] = "TIME", a[a.ELAPSED = 7] = "ELAPSED", a[a.POWER = 8] = "POWER", a[a.P_CYCLIST_RAW = 9] = "P_CYCLIST_RAW", a[a.P_CYCLIST_WHEEL = 10] = "P_CYCLIST_WHEEL", a[a.P_CYCLIST_OPTIMAL_POWER = 11] = "P_CYCLIST_OPTIMAL_POWER", a[a.P_AERO = 12] = "P_AERO", a[a.P_GRAVITY = 13] = "P_GRAVITY", a[a.P_ROLLING_RESISTANCE = 14] = "P_ROLLING_RESISTANCE", a[a.P_WHEEL_BEARINGS = 15] = "P_WHEEL_BEARINGS", a[a.P_POWER_FROM_ACC = 16] = "P_POWER_FROM_ACC", a[a.P_POWER_WHEEL_FROM_ACC = 17] = "P_POWER_WHEEL_FROM_ACC", a[a.AERO_COEF = 18] = "AERO_COEF", a[a.GRADE = 19] = "GRADE", a[a.SPEED = 20] = "SPEED", a[a.SPEED_MAX = 21] = "SPEED_MAX", a[a.SPEED_MAX_INCLINE = 22] = "SPEED_MAX_INCLINE", a[a.VIRT_SPEED_CURRENT = 23] = "VIRT_SPEED_CURRENT", a[a.TEMPERATURE = 24] = "TEMPERATURE", a[a.WIND_SPEED = 25] = "WIND_SPEED", a[a.WIND_DIRECTION = 26] = "WIND_DIRECTION", a[a.WIND_BEARING = 27] = "WIND_BEARING", a[a.WIND_ALPHA = 28] = "WIND_ALPHA", a[a.HEART_RATE = 29] = "HEART_RATE", a[a.CADENCE = 30] = "CADENCE", a))(h || {}), at = /* @__PURE__ */ ((a) => (a.lat = "lat", a.lon = "lon", a.ele = "ele", a.bearing = "bearing", a.dist = "dist", a.radius = "radius", a.time = "time", a.elapsed = "elapsed", a.power = "power", a.pCyclistRaw = "pCyclistRaw", a.pCyclistWheel = "pCyclistWheel", a.pCyclistOptimalPower = "pCyclistOptimalPower", a.pAero = "pAero", a.pGravity = "pGravity", a.pRollingResistance = "pRollingResistance", a.pWheelBearings = "pWheelBearings", a.pPowerFromAcc = "pPowerFromAcc", a.pPowerWheelFromAcc = "pPowerWheelFromAcc", a.aeroCoef = "aeroCoef", a.grade = "grade", a.speed = "speed", a.speedMax = "speedMax", a.speedMaxIncline = "speedMaxIncline", a.virtSpeedCurrent = "virtSpeedCurrent", a.temperature = "temperature", a.windSpeed = "windSpeed", a.windDirection = "windDirection", a.windBearing = "windBearing", a.windAlpha = "windAlpha", a.heartRate = "heartRate", a.cadence = "cadence", a))(at || {});
const Xt = {
...
class ct {
   ...
   */
  time(e) {
    this.doTime(M.INFO, e);
  }
  /**
   * End timing and log duration
   */
  timeEnd(e) {
    this.doTimeEnd(M.INFO, e);
  }
  logDir(e, t, s, i) {
    this.doLog(e, "DIR %s", t), console.dir(s, i);
  }
...
class P {
   ...
   */
  multiply(e) {
    return new P(this.x * e, this.y * e, this.z * e);
  }
  /**
   * Calculate dot product with another vector
   */
  ...
  normalize() {
    const e = this.magnitude();
    return e === 0 ? new P(0, 0, 0) : this.multiply(1 / e);
  }
  /**
   * Calculate perpendicular distance from this point to a line segment defined by two points
   * Uses the formula: ||(p-a) × (p-b)|| / ||b-a||
    ...
    if (this.totalDistance = 0, this.timeStart = 0, this.minElevation = Number.MAX_VALUE, this.maxElevation = -Number.MAX_VALUE, this.totalElevationGain = 0, this.totalElevationLoss = 0, this.minLat = Number.MAX_VALUE, this.maxLat = -Number.MAX_VALUE, this.minLon = Number.MAX_VALUE, this.maxLon = -Number.MAX_VALUE, this.pointCount !== 0) {
      ...
      for (let e = 0; e < this.pointCount; e++) {
        ...
        if (r > 0) {
          ...
          const d = this.getLatitude(e), u = this.getLongitude(e), g = this.getLatitude(n), m = this.getLongitude(n), f = this.project(d, u), C = this.project(g, m), R = C.y - f.y, T = C.x - f.x, v = Math.atan2(-R, T);
          this.setBearing(e, v);
        } else
...
class mt {
  ...
  static computeMaxSpeedByIncline(e, t, s, i) {
    const n = e.path, r = e.cyclist, o = this.transform(n, t, s), l = new P(0, 0, 0), c = this.transform(n, i, s), d = this.getCircleCenter(o, l, c);
    if (d === null) {
  ...
  static getCircleCenter(e, t, s) {
    const i = e.x, n = e.y, r = t.x, o = t.y, l = s.x, c = s.y, d = r - i, u = o - n, g = l - i, m = c - n, f = d * (i + r) + u * (n + o), C = g * (i + l) + m * (n + c), R = 2 * (d * (c - o) - u * (l - r));
    if (Math.abs(R) < 1e-3)
      return null;
    const T = (m * f - u * C) / R, v = (d * C - g * f) / R;
    return new P(T, v, 0);
  }
  /**
...
class Et {
  ...
  computePAirWithWind(e, t, s, i) {
    ...
    const c = i.windSpeed, d = n + c * Math.cos(l), u = d * d, g = n * n + c * c + 2 * n * c * Math.cos(l), m = u / g, C = m + 1.2 * (1 - m);
    return -s * C * Math.sqrt(g) * d * n;
  }
}
const wt = new Et();
class ft {
...
const Nt = new Mt(), L = class L {
  ...
  constructor() {
    ...
    for (let t = 0; t < e.length; t++)
      e[t] = Math.floor(Math.random() * 4294967295);
    for (let t = 0; t < 20; t++) {
      const s = 1 + e[t * 3] / 4294967295 * 9, i = e[t * 3 + 1] / 4294967295 * Math.PI, n = e[t * 3 + 2] / 4294967295 * 0.01;
      this.harmonics.push({ freq: s, d: i, amp: n });
    }
  }
  /**
  ...
  getPowerW(e, t, s) {
    ...
    if (e.cyclist.harmonics) {
      const r = t.getTime(s) / 1e4;
      for (const o of this.harmonics)
        i += o.amp * i * Math.cos(o.freq * r - o.d);
    }
    return t.setField(s, h.P_CYCLIST_OPTIMAL_POWER, i), this.getRealOptimalPower(e, t, s, i);
  }
  getRealOptimalPower(e, t, s, i) {
    const n = Math.max(
      0,
      -Y.INSTANCE.getNewPower(e, t, s, !1)
    ), r = i * (1 - L.TOLERANCE), o = i * (1 + L.TOLERANCE);
    if (r <= n && n <= o)
      return i;
    if (n < r)
      return i * L.MAX_MULTIPLIER - n / r * i * (L.MAX_MULTIPLIER - 1);
    {
...
L.TOLERANCE = 0.05, L.MAX_MULTIPLIER = 3;
let K = L;
class Pt {
  /**
...
class qt extends K {
   ...
   */
  getOptimalPower(e, t, s) {
    const i = e.cyclist.power, n = t.getElapsed(s) / 1e3, r = Math.max(0.5, 1 - 0.6 * n / this.duration);
    return i * r;
  }
}
class St {
...
const bt = new It(), Q = class Q {
  ...
  getNewPower(e, t, s, i) {
    let n = 0;
    return n += bt.getPowerW(e, t, s), n += Dt.getPowerW(e, t, s), n += wt.getPowerW(e, t, s), n += xt.getPowerW(e, t, s), i && (n += Tt.getPowerW(e, t, s)), n;
  }
  /**
...
Q.INSTANCE = new Q();
let Y = Q;
const Ot = Ne("physics/VirtualService"), oe = class oe {
  /**
  ...
  static virtualizeTrack(e) {
    ...
    for (s.addPoint({
      ...
      const m = this.getIndex(n, r, c), f = this.powerComputer.getNewPower(e, s, s.length - 1, !0), C = this.powerComputer.getDx(f, t, d, te), R = this.getNextIndex(n, r, c, C);
      let T, v;
      if (m !== R)
        T = i.getDistance(R) - c, v = this.powerComputer.getDt(f, t, d, T), l = i.getPointData(R), c = i.getDistance(R);
      else {
        T = C, v = te;
        const ce = c + C, he = i.getDistance(m), Te = i.getDistance(m + 1), ve = (ce - he) / (Te - he);
        l = i.interpolatePoint(m, m + 1, ve), c = ce;
      }
      let b = 2 * (T / v) - d;
      const le = l.speedMax;
      if (b > le && (b = le, v = 2 * T / (d + b)), d = b, u += v * 1e3, s.addPoint({
        ...l,
        dist: c,
        time: u,
        elapsed: u - g,
        ...
        Ot.warn("VirtualizeService: Simulation exceeded 10x original points, stopping");
        break;
      }
    }
...
class J {
   ...
   */
  static toEcef(e, t = 3) {
    const s = e.lat, i = e.lon, n = isNaN(e.ele) ? 0 : e.ele, r = t * n, o = Math.sin(s), l = Ae / Math.sqrt(1 - ue * o * o), c = Math.cos(s), d = Math.cos(i), u = Math.sin(i), g = (l + r) * c * d, m = (l + r) * c * u, f = (l * (1 - ue) + r) * o;
    return new P(g, m, f);
  }
}
class Wt {
...
class ae {
   ...
   */
  constructor(e, t, s, i, n, r, o, l) {
    this.mKg = e, this.power = t, this.harmonics = s, this.maxBrakeG = i, this.cd = n, this.a = r, this.maxAngleDeg = o, this.maxSpeedKmH = l;
  }
  /**
  ...
  static getDefault() {
    return new ae(
      ...
      Ie,
      !1,
      // harmonics - not a physical parameter
      be,
      We,
      Fe,
      Ce,
...
class Kt {
  static getDefaultCourse(e) {
    return {
      ...
      aeroProvider: pt,
      windProvider: Nt,
      cyclistPowerProvider: yt
    };
  }
  static async enhancePath(e) {
    return this.enhance(this.getDefaultCourse(e));
  }
  static async enhance(e) {
    ee.timeLevel(M.INFO, "enhance"), ee.info(e);
    let t = await dt.fixElevation(e.path);
    const s = { ...e, path: t };
    return mt.computeMaxSpeeds(s), t = ie.virtualizeTrack(s), t = Ft.computeOnePointPerSecond(t), t = Wt.simplify(t, 10, 3), ee.timeEndLevel(M.INFO, "enhance"), t;
  }
}
const w = {
...
class Pe {
  ...
  parse(e) {
    ...
    this.namespaceResolver = new Bt(s), this.extensionParser = new Ut(this.namespaceResolver);
   ...
   */
  parseTrackPoint(e) {
    const t = { ...Me }, s = e.getAttribute("lat"), i = e.getAttribute("lon");
    if (!s || !i)
...
export {
  ...
  Le as DEFAULT_INERTIA_REAR,
  be as DEFAULT_MAX_BRAKE_G,
  Ce as DEFAULT_MAX_LEAN_ANGLE_DEG,
  zt as DEFAULT_MAX_LEAN_ANGLE_RAD,
  Oe as DEFAULT_MAX_SPEED_KMH,
  xe as DEFAULT_WHEEL_RADIUS,
  te as DT,
  Wt as DouglasPeucker,
  Me as EMPTY_POINT,
  J as EcefConverter,
  dt as Elevation,
  Kt as Enhancer,
  ...
  Ft as PointPerSecond,
  Y as PowerComputer,
  qt as PowerProviderConstantWithTiring,
  Ae as SEMI_MAJOR_AXIS,
  P as Vector3D,
  ie as VirtualizeService,
  Rt as WindProviderConstant,
  wt as aeroPowerProvider,
  pt as aeroProviderConstant,
  Ne as createLogger,
  Xt as fieldToPointField,
  xt as gravPowerProvider,
  Tt as muscularPowerProvider,
  yt as powerProviderConstant,
  Ht as powerProviderFromData,
  Vt as rhoProviderDefault,
  Ct as rhoProviderEstimate,
  Dt as rollingResistancePowerProvider,
  F as toDegrees,
  H as toRadians,
  bt as wheelBearingsPowerProvider,
